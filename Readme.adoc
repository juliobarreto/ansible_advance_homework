==== To review:
In the file openstack.pem, in the workstation machine, there are the following lines at the beginning and at the end of the key:
#Begin Ansible Managed Block
#End Ansible Managed Block
These lines produce an error when running the role config_tower, when the openstack.pem file tries to be entered in tower as a credential value.
You must edit the openstack.pem file and delete both lines

In the playbook aws_status_check.yml the first line is configured to filter the instances associated with nelson.melo@seaq.co:
hosts: tag_instance_filter_three_tier_app_nelson_melo_seaq_co
